...
```mermaid
graph TB
APP["应用入口<br/>src/main.ts"] --> ROUTER["路由模块<br/>src/router/auth.ts"]
APP --> STORE["状态模块<br/>src/store/auth.ts + sys_user.ts"]
APP --> DIRECTIVES["指令模块<br/>src/directives/auth.ts + permission.ts"]
ROUTER --> LAYOUT["布局组件<br/>src/layout/index.vue"]
LAYOUT --> VIEWS["视图组件<br/>login/dashboard/system/user"]
STORE --> TYPES["类型模块<br/>src/types/auth.ts"]
VIEWS --> STORE
VIEWS --> ROUTER
...
```mermaid
graph LR
MAIN["main.ts"] --> ROUTER["router/auth.ts"]
MAIN --> PINIA["store/auth.ts"]
MAIN --> DIRECTIVES["directives/auth.ts"]
ROUTER --> USERSTORE["store/sys_user.ts"]
...
- 关键文件清单
  ...
  - 路由定义与守卫：[src/router/auth.ts](file://src/router/index.ts#L1-L123)
  - 状态管理：[src/store/auth.ts](file://src/store/index.ts#L1-L6)、[src/store/sys_user.ts](file://src/store/user.ts#L1-L68)
  - 指令注册与实现：[src/directives/auth.ts](file://src/directives/index.ts#L1-L16)、[src/directives/permission.ts](file://src/directives/permission.ts#L1-L67)
  - 类型定义：[src/types/auth.ts](file://src/types/index.ts#L1-L45)
