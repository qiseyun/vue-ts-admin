...
ENTRY --> ROUTER["路由插件<br/>src/router/index.ts"]
ENTRY --> PINIA["状态插件<br/>src/store/index.ts"]
ENTRY --> DIRS["全局指令注册<br/>src/directives/index.ts"]
EP --> UI["UI 组件库"]
ROUTER --> NAV["导航与守卫"]
PINIA --> STORE["用户状态<br/>src/store/user.ts"]
...
participant Pinia as "src/store/index.ts"
participant Directives as "src/directives/index.ts"
Browser->>Main : 加载入口脚本
Main->>Main : 创建应用实例
Main->>EP : 注册插件与图标
Main->>Router : 注册路由插件
...
章节来源
- [src/router/index.ts](file://src/router/index.ts#L10-L11)
