...
```mermaid
graph TB
A["应用入口<br/>src/main.ts"] --> B["路由模块<br/>src/router/auth.ts"]
A --> C["状态管理<br/>src/store/auth.ts"]
...
Store["状态管理<br/>src/store/auth.ts"]
...
participant S as "状态管理<br/>src/store/auth.ts"
...
- [src/router/auth.ts](file://src/router/index.ts#L92-L107)
