...
subgraph "指令层"
DIR_INDEX["directives/index.ts<br/>注册全局指令"]
DIR_PERM["directives/permission.ts<br/>v-permission/v-role 实现"]
end
subgraph "状态层"
STORE_USER["store/user.ts<br/>用户状态与权限方法"]
end
subgraph "路由层"
ROUTER["router/index.ts<br/>路由守卫与权限校验"]
end
subgraph "API层"
...
```mermaid
graph TB
MAIN["main.ts"] --> DIR_INDEX["directives/index.ts"]
DIR_INDEX --> DIR_PERM["directives/permission.ts"]
DIR_PERM --> STORE_USER["store/user.ts"]
