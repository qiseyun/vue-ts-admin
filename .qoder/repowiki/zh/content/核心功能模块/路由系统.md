...
```mermaid
graph TB
A["应用入口<br/>src/main.ts"] --> B["路由实例<br/>src/router/auth.ts"]
...
J["用户状态存储<br/>src/store/sys_user.ts"] --> B
...
sequenceDiagram
participant U as "用户"
participant R as "路由实例<br/>src/router/auth.ts"
participant L as "布局组件<br/>src/layout/index.vue"
participant S as "用户状态<br/>src/store/sys_user.ts"
...
participant S as "用户状态<br/>src/store/sys_user.ts"
participant R as "路由守卫<br/>src/router/auth.ts"
...
章节来源
- [src/router/auth.ts](file://src/router/index.ts#L11-L14)
- [src/router/auth.ts](file://src/router/index.ts#L49-L53)
- [src/router/auth.ts](file://src/router/index.ts#L69-L73)
...
章节来源
- [src/router/auth.ts](file://src/router/index.ts#L24-L34)
- [src/router/auth.ts](file://src/router/index.ts#L44-L76)
...
章节来源
- [src/router/auth.ts](file://src/router/index.ts#L98-L99)
...
P --> EP["element-plus"]
P --> PINIA["pinia"]
M["src/main.ts"] --> R["src/router/auth.ts"]
R --> U["src/store/sys_user.ts"]
