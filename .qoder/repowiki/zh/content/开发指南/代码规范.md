...
- 类型定义集中于 types/index.ts，便于跨模块复用
...
- 类型定义最佳实践
  - 在 types/index.ts 定义通用接口（如 UserInfo、LoginForm、ApiResponse、MenuItem）
...
- 命名约定
  ...
  - 参考实现位置：[src/router/index.ts](file://src/router/index.ts#L6-L86)
...
- 接口设计
  ...
  - 参考实现位置：[src/types/index.ts](file://src/types/index.ts#L1-L45)

- 泛型使用
  - ApiResponse<T> 支持泛型数据承载，避免 any
  - request.ts 中的泛型函数封装，确保返回值类型安全
  - 参考实现位置：
    - [src/types/index.ts](file://src/types/index.ts#L25-L30)
...
- 错误：使用 any 替代具体类型
  - 路径参考：[src/types/index.ts](file://src/types/index.ts#L26-L30)（可对比使用泛型的 ApiResponse）
- 正确：组件命名与路由配置一致
  - 路径参考：[src/router/index.ts](file://src/router/index.ts#L46-L74)、[src/views/system/user/index.vue](file://src/views/system/user/index.vue#L1-L183)
...
- 构建与懒加载
  - 路由与视图采用动态导入，减少首屏体积
  - 参考实现位置：[src/router/index.ts](file://src/router/index.ts#L10-L85)
...
- 快速检查清单
  ...
  - 新增组件遵循 PascalCase 命名与路由/视图文件夹一致性
    - 参考实现位置：[src/router/index.ts](file://src/router/index.ts#L46-L74)
  - 使用泛型封装 API 响应，避免 any
    - 参考实现位置：[src/types/index.ts](file://src/types/index.ts#L25-L30)