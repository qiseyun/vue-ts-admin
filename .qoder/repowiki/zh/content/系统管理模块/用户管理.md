...
- 权限指令：src/directives/permission.ts、src/directives/auth.ts
- 状态管理：src/store/sys_user.ts
- 类型定义：src/types/auth.ts
- 网络请求封装：src/utils/request.ts
- 路由与导航：src/router/auth.ts、src/layout/index.vue
- 应用入口：src/main.ts
...
DI["directives/auth.ts"]
end
subgraph "状态层"
SU["store/sys_user.ts"]
end
subgraph "类型与工具"
TY["types/auth.ts"]
UT["utils/request.ts"]
...
章节来源
- [src/views/system/user/index.vue](file://src/views/system/user/index.vue#L1-L183)
- [src/router/auth.ts](file://src/router/index.ts#L36-L76)
- [src/layout/index.vue](file://src/layout/index.vue#L1-L255)
...
IDX --> STORE["store/sys_user.ts"]
IDX --> TYPES["types/auth.ts"]
IDX --> REQ["utils/request.ts"]
PERM --> STORE
ROUTE["router/auth.ts"] --> STORE
