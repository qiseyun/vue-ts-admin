...
subgraph "状态层"
STORE["用户状态存储<br/>src/store/user.ts"]
end
subgraph "权限层"
...
- [src/types/index.ts](file://src/types/index.ts#L2-L44)
...
participant S as "用户状态存储<br/>store/user.ts"
...
```mermaid
graph LR
ROLE["role/index.vue"] --> STORE["store/user.ts"]
ROLE --> DIRECTIVES["directives/permission.ts"]
ROLE --> ROUTER["router/index.ts"]
ROLE --> TYPES["types/index.ts"]
