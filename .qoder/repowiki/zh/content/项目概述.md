...
A --> C["src/router/auth.ts<br/>路由配置"]
A --> D["src/store/auth.ts<br/>状态管理入口"]
...
A --> J["src/directives/auth.ts<br/>指令注册"]
J --> K["src/directives/permission.ts<br/>权限/角色指令"]
A --> L["src/types/auth.ts<br/>类型定义"]
```
...
graph TB
subgraph "运行时"
M["main.ts<br/>应用装配"] --> R["router/auth.ts<br/>路由守卫"]
M --> S["store/auth.ts<br/>Pinia"]
M --> L["layout/index.vue<br/>布局"]
M --> U["utils/request.ts<br/>HTTP"]
M --> D["directives/auth.ts<br/>指令注册"]
end
subgraph "视图层"
...
- [src/router/auth.ts](file://src/router/index.ts#L98-L103)
